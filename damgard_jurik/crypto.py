...
class EncryptedNumber:
    ...
    def __add__(self, other: Any) -> "EncryptedNumber":
        ...
        if not isinstance(other, EncryptedNumber):
            raise ValueError(
                "Can only add/subtract an EncryptedNumber to another EncryptedNumber"
            )

        if self.public_key != other.public_key:
            raise ValueError(
                "Attempted to add/subtract numbers encrypted against different public keys!"
            )
    ...
    def __sub__(self, other: Any) -> "EncryptedNumber":
        ...
        if not isinstance(other, EncryptedNumber):
            raise ValueError(
                "Can only add/subtract an EncryptedNumber from another EncryptedNumber"
            )
    ...
    def __rsub__(self, other: Any) -> "EncryptedNumber":
        ...
        self_inv = EncryptedNumber(
            value=inv_mod(self.value, self.public_key.n_s_1), public_key=self.public_key
        )
...
class PrivateKeyRing:
    def __init__(self, private_key_shares: List[PrivateKeyShare]):
        ...
        if len(private_key_shares) < public_key.threshold:
            raise ValueError(
                "Number of unique PrivateKeyShares is less than the threshold to decrypt"
            )

        self.public_key = public_key
        self.private_key_shares = list(private_key_shares)
        self.i_list = [pks.i for pks in self.private_key_shares]
        self.S = set(self.i_list)
        self.inv_four_delta_squared = inv_mod(
            4 * (self.public_key.delta**2), self.public_key.n_s
        )
    ...
    def decrypt(self, c: EncryptedNumber) -> int:
        ...
        def lam(i: int) -> int:
            ...
            for i_prime in S_prime:
                l = (
                    l
                    * i_prime
                    * inv_mod(i_prime - i, self.public_key.n_s_m)
                    % self.public_key.n_s_m
                )
        ...
        for c_i, i in zip(c_list, self.i_list):
            c_prime = (
                c_prime * pow_mod(c_i, (2 * lam(i)), self.public_key.n_s_1)
...
def keygen(
    n_bits: int = 64, s: int = 1, threshold: int = 3, n_shares: int = 3
    ...
    if n_shares < threshold:
        raise ValueError(
            "The number of shares must be at least as large as the threshold"
        )
    ...
    shares = share_secret(
        secret=d, modulus=n_s_m, threshold=threshold, n_shares=n_shares
    )
